...
/* バッジの背景色を themeColor に連動 */
.badge {
  background: v-bind('globalSettings.themeColor');
  color: white;
  padding: 4px 12px;
  border-radius: 20px;
  font-size: 0.9rem;
...
/* ホバー時の枠線と背景を themeColor に連動（color-mixで10%の薄い色を生成） */
.option-btn:hover:not(:disabled) {
  border-color: v-bind('globalSettings.themeColor');
  background: color-mix(in srgb, v-bind('globalSettings.themeColor') 10%, transparent);
}

/* 選択時の枠線と背景を themeColor に連動（color-mixで20%の薄い色を生成） */
.option-btn.selected {
  border-color: v-bind('globalSettings.themeColor');
  background: color-mix(in srgb, v-bind('globalSettings.themeColor') 20%, transparent);
}
